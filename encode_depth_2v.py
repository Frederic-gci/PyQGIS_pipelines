...
bands = [[np.full(shape=wse.shape, fill_value=255, dtype=np.uint8)
         for i in range(len(classes))] for j in range(v2_length)]
...
for v2 in range(v2_length):
    for v1 in range(v1_length): 
        ...
        for class_idx in range(len(classes)):
            bands[v2][class_idx] = np.where(
                np.greater(data, mnt_rect + classes[class_idx]),
                np.minimum(bands[v2][class_idx], (v1 + 1)),
                bands[v2][class_idx])
    for class_idx in range(len(classes)):
        ...
        output_file.write(bands[v2][class_idx], 1)
        output_file.close
